<!-- productQMX引用组件 订单提交成功 wuxiaobo-->
<template>
  <div class="">
    <embed :src="pdfUrl"></embed>
    <PointOut v-show="pointShow" />
  </div>
  <!-- <div id="container" >
    <div class='pdf-page'>
      <div class="pageBoxWarp">
        <div class="pageBox">
          第{{ pageNum }}页/ 总{{ pdfDoc.numPages }}页
        </div>
      </div>
      <canvas id="the-canvas"></canvas>
      <div class="foot" v-if='pdfDoc'>
        <button class='left' v-if="pageNum>1" @click="onPrevPage">上一页</button>
        <button class='right' v-if="pageNum<pdfDoc.numPages" @click="onNextPage">下一页</button>
      </div>
    </div>
    <PointOut v-show="pointShow" />
  </div> -->
</template>

<script>
import PointOut from '@/components/PointOut'
import {mapState} from 'vuex'

export default {
  name: 'QmxPdf',
  data () {
    return {
      
    }
  },
  computed: {
    ...mapState({
      // 获取数据
      pointShow: state => state.pointShow,
      pdfUrl: state => state.pdfUrl
    })
  },
  watch: {},
  mounted () {
    // 钩子函数
    this.init()
  },
  methods: {
    init () {

    }
  },
  components: {
    PointOut
　}
}


// import PointOut from '@/components/PointOut'
// import {mapState} from 'vuex'
// import PDFJS from 'pdfjs-dist'
//
// export default {
//   name: 'QmxPdf',
//   data () {
//     return {
//       pdfDoc: null,
//       pageNum: 1,
//       pageRendering: false,
//       pageNumPending: null,
//       scale: 0.9
//     }
//   },
//   computed: {
//     ...mapState({
//       // 获取数据
//       pointShow: state => state.pointShow
//     })
//   },
//   watch: {},
//   mounted () {
//     // 钩子函数
//     this.init()
//   },
//   methods: {
//     init () {
//       this.showPDF('../staticV2/pdf/qimingxing.pdf')
//     },
//     showPDF (url) {
//       let _this = this
//       PDFJS.getDocument(url).then(function (pdf) {
//         _this.pdfDoc = pdf
//         _this.renderPage(1)
//       })
//     },
//     renderPage (num) {
//       this.pageRendering = true
//       let _this = this
//       this.pdfDoc.getPage(num).then(function (page) {
//         var viewport = page.getViewport(_this.scale)
//         let canvas = document.getElementById('the-canvas')
//         canvas.height = viewport.height
//         canvas.width = viewport.width
//
//         // Render PDF page into canvas context
//         var renderContext = {
//           canvasContext: canvas.getContext('2d'),
//           viewport: viewport
//         }
//         var renderTask = page.render(renderContext)
//
//         // Wait for rendering to finish
//         renderTask.promise.then(function () {
//           _this.pageRendering = false
//           if (_this.pageNumPending !== null) {
//             // New page rendering is pending
//             this.renderPage(_this.pageNumPending)
//             _this.pageNumPending = null
//           }
//         })
//       })
//     },
//     queueRenderPage (num) {
//       if (this.pageRendering) {
//         this.pageNumPending = num
//       } else {
//         this.renderPage(num)
//       }
//     },
//     onPrevPage () {
//       if (this.pageNum <= 1) {
//         return
//       }
//       this.pageNum--
//       this.queueRenderPage(this.pageNum)
//     },
//     onNextPage () {
//       if (this.pageNum >= this.pdfDoc.numPages) {
//         return
//       }
//       this.pageNum++
//       this.queueRenderPage(this.pageNum)
//     }
//   },
//   components: {
//     PointOut
// 　}
// }

</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
embed{
  width: 100vw;height: 100vh;
}
#container {
  background-color: rgba(0,0,0,0.75);width: 100%;min-height: 798px;
  height: 100vh;text-align: center;padding: 5px;}
.pageBoxWarp{height: 30px;}
.pageBox{position: absolute;top: 10px;width: 100%;text-align: center;color: #fff;}
.pdf-page {}
.foot {position: fixed;  transform: translate(-50%,0);left: 50%;bottom: 10px;}
</style>
